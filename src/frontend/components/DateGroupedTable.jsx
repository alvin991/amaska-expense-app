...
function DateGroupedTable({ data, onRowDoubleClick }) {
    ...
    // console.log(`DateGroupedTable data ${JSON.stringify(data, null, 2)}`);
    ...
    const groupedData = Object.values(
        ...
        // console.log(currentItem)
            // const { transaction_date, amount } = currentItem['date'];
            const { date, amount } = currentItem;
            // console.log(`date ${date} amount ${date}`)
            // console.log(`Processing item: date=${transaction_date}, amount=${amount}`);
            // If the date is not yet in the accumulator, create a new entry
            if (!accumulator[date]) {
                accumulator[date] = {
                        ...
                        <React.Fragment key={group.date}>
                            ...
                            {group.items.map((item) => (
                                <tr key={item.transaction_id} onDoubleClick={() => onRowDoubleClick?.(item.transaction_id)}>
                                    ...
                                    <td style={{ width: columnWidths.category }}>{item.category}</td>
                                    <td style={{ width: columnWidths.paymentMethod }}>{item.paymentMethod}</td>
