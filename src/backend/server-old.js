...
// Initialize SYSTEM user at startup
...
function applyRecurringTemplates(upToDate, callback) {
    ...
    db.all('SELECT * FROM recurring_templates where enabled = 1', [], (err, rows) => {
        if (err) {
...
// Schedule cron job to apply recurring templates daily
...
// Test endpoint to check cron status
apiRouter.get('/cron/status', (req, res) => {
    res.json({
